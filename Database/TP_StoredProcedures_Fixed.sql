...
EXISTS (
    SELECT 1 
    FROM @CuisinesTable CT 
    WHERE R.Cuisine LIKE '%' + CT.CuisineName + '%'
) -- Match any cuisine in the list using LIKE for partial matches
)
ORDER BY
OverallRating DESC,
ReviewCount DESC;
...
-- Get all unique cuisines for dropdown
CREATE PROCEDURE dbo.TP_spGetAllCuisines
AS
BEGIN
    SET NOCOUNT ON;
    BEGIN TRY
        -- Create a temporary table to hold split cuisines
        CREATE TABLE #TempCuisines (Cuisine NVARCHAR(100));
        
        -- Insert split cuisines into temp table
        INSERT INTO #TempCuisines (Cuisine)
        SELECT DISTINCT TRIM(value) as Cuisine
        FROM dbo.TP_Restaurants
        CROSS APPLY STRING_SPLIT(Cuisine, ',')
        WHERE Cuisine IS NOT NULL AND TRIM(Cuisine) <> '';
        
        -- Return distinct, cleaned cuisines
...
-- Add sample data for restaurants, users, and reviews
BEGIN
    -- First, add restaurant representative users
    INSERT INTO dbo.TP_Users (
        Username,
        Email,
        PasswordHash,
    ...
    VALUES 
    ('nycpizza_rep', 'nycpizza@example.com', 'AQAAAAIAAYagAAAAELwGqj8B6kBfKpt4kDePOyHgwg8tQlzNbkY2aUXDV6xHVny+VZVKrPDGatJHGxhxYw==', 'restaurantRep', GETDATE(),
    'What is your favorite color?', 'blue_hash', 'What is your pet name?', 'max_hash', 'What is your hometown?', 'nyc_hash', 1),
    
    ('texasbbq_rep', 'texasbbq@example.com', 'AQAAAAIAAYagAAAAELwGqj8B6kBfKpt4kDePOyHgwg8tQlzNbkY2aUXDV6xHVny+VZVKrPDGatJHGxhxYw==', 'restaurantRep', GETDATE(),
    'What is your favorite color?', 'red_hash', 'What is your pet name?', 'buddy_hash', 'What is your hometown?', 'austin_hash', 1),
    
    ('lasushi_rep', 'lasushi@example.com', 'AQAAAAIAAYagAAAAELwGqj8B6kBfKpt4kDePOyHgwg8tQlzNbkY2aUXDV6xHVny+VZVKrPDGatJHGxhxYw==', 'restaurantRep', GETDATE(),
    'What is your favorite color?', 'green_hash', 'What is your pet name?', 'luna_hash', 'What is your hometown?', 'la_hash', 1),
    
    ('chicagopasta_rep', 'chicagopasta@example.com', 'AQAAAAIAAYagAAAAELwGqj8B6kBfKpt4kDePOyHgwg8tQlzNbkY2aUXDV6xHVny+VZVKrPDGatJHGxhxYw==', 'restaurantRep', GETDATE(),
    'What is your favorite color?', 'yellow_hash', 'What is your pet name?', 'rocky_hash', 'What is your hometown?', 'chicago_hash', 1),
    
    ('miamiseafood_rep', 'miamiseafood@example.com', 'AQAAAAIAAYagAAAAELwGqj8B6kBfKpt4kDePOyHgwg8tQlzNbkY2aUXDV6xHVny+VZVKrPDGatJHGxhxYw==', 'restaurantRep', GETDATE(),
    'What is your favorite color?', 'blue_hash', 'What is your pet name?', 'coco_hash', 'What is your hometown?', 'miami_hash', 1);
    ...
    VALUES 
    ('foodie_john', 'john@example.com', 'AQAAAAIAAYagAAAAELwGqj8B6kBfKpt4kDePOyHgwg8tQlzNbkY2aUXDV6xHVny+VZVKrPDGatJHGxhxYw==', 'reviewer', GETDATE(),
    'What is your favorite color?', 'purple_hash', 'What is your pet name?', 'spot_hash', 'What is your hometown?', 'boston_hash', 1),
    
    ('taste_sarah', 'sarah@example.com', 'AQAAAAIAAYagAAAAELwGqj8B6kBfKpt4kDePOyHgwg8tQlzNbkY2aUXDV6xHVny+VZVKrPDGatJHGxhxYw==', 'reviewer', GETDATE(),
    'What is your favorite color?', 'pink_hash', 'What is your pet name?', 'bella_hash', 'What is your hometown?', 'seattle_hash', 1),
    
    ('critic_mike', 'mike@example.com', 'AQAAAAIAAYagAAAAELwGqj8B6kBfKpt4kDePOyHgwg8tQlzNbkY2aUXDV6xHVny+VZVKrPDGatJHGxhxYw==', 'reviewer', GETDATE(),
    'What is your favorite color?', 'black_hash', 'What is your pet name?', 'zeus_hash', 'What is your hometown?', 'denver_hash', 1);

    -- Add restaurants (using UserIDs from the restaurant representatives)
    INSERT INTO dbo.TP_Restaurants (
        RestaurantID,  -- This matches the UserID of the representative
        Name,
        Address,
        City,
        State,
    ...
    SELECT UserID, 'NYC Pizza Express', '123 Broadway', 'New York', 'NY', '10007', 'Italian, Pizza', '11:00 AM - 10:00 PM', '(212) 555-0123', 'pizza_profile.jpg',
           'Authentic New York-style pizza made with traditional recipes and the finest ingredients.', GETDATE()
    FROM dbo.TP_Users WHERE Username = 'nycpizza_rep'
    UNION ALL
    SELECT UserID, 'Texas BBQ House', '456 Austin Ave', 'Austin', 'TX', '78701', 'BBQ, American', '11:00 AM - 9:00 PM', '(512) 555-0456', 'bbq_profile.jpg',
           'Real Texas BBQ smoked to perfection. Family recipes passed down through generations.', GETDATE()
    FROM dbo.TP_Users WHERE Username = 'texasbbq_rep'
    UNION ALL
    SELECT UserID, 'LA Sushi Bar', '789 Sunset Blvd', 'Los Angeles', 'CA', '90028', 'Japanese, Sushi', '12:00 PM - 11:00 PM', '(323) 555-0789', 'sushi_profile.jpg',
           'Premium sushi and Japanese cuisine using fresh, locally-sourced seafood.', GETDATE()
    FROM dbo.TP_Users WHERE Username = 'lasushi_rep'
    UNION ALL
    SELECT UserID, 'Chicago Pasta House', '321 Michigan Ave', 'Chicago', 'IL', '60601', 'Italian', '11:00 AM - 10:00 PM', '(312) 555-0321', 'pasta_profile.jpg',
           'Authentic Italian pasta made fresh daily. A taste of Italy in the heart of Chicago.', GETDATE()
    FROM dbo.TP_Users WHERE Username = 'chicagopasta_rep'
    UNION ALL
    SELECT UserID, 'Miami Seafood Grill', '555 Ocean Drive', 'Miami', 'FL', '33139', 'Seafood', '12:00 PM - 11:00 PM', '(305) 555-0555', 'seafood_profile.jpg',
           'Fresh-caught seafood prepared with Caribbean flair. Ocean views and tropical atmosphere.', GETDATE()
    FROM dbo.TP_Users WHERE Username = 'miamiseafood_rep';

    -- Add reviews from the reviewers
    -- First get the UserIDs for reviewers
    DECLARE @JohnID INT = (SELECT UserID FROM dbo.TP_Users WHERE Username = 'foodie_john');
    DECLARE @SarahID INT = (SELECT UserID FROM dbo.TP_Users WHERE Username = 'taste_sarah');
    DECLARE @MikeID INT = (SELECT UserID FROM dbo.TP_Users WHERE Username = 'critic_mike');
    ...
    SELECT 
        ...
        'Amazing authentic flavors! The crust was perfect.',
        5, 4, 4, 3,
        GETDATE()
    FROM dbo.TP_Restaurants WHERE Name = 'NYC Pizza Express'
    UNION ALL
    SELECT 
        RestaurantID,
        @SarahID,
        DATEADD(day, -3, GETDATE()),
        'Best BBQ in Texas! The brisket was incredibly tender.',
        5, 5, 4, 4,
        GETDATE()
    FROM dbo.TP_Restaurants WHERE Name = 'Texas BBQ House'
    UNION ALL
    SELECT 
        RestaurantID,
        @MikeID,
        DATEADD(day, -7, GETDATE()),
        'Fresh and innovative sushi rolls. Great atmosphere!',
        4, 5, 5, 4,
        GETDATE()
    FROM dbo.TP_Restaurants WHERE Name = 'LA Sushi Bar'
    UNION ALL
    SELECT 
        RestaurantID,
        @JohnID,
        DATEADD(day, -2, GETDATE()),
        'Homemade pasta that rivals any in Italy. Will definitely return!',
        5, 4, 4, 3,
        GETDATE()
    FROM dbo.TP_Restaurants WHERE Name = 'Chicago Pasta House'
    UNION ALL
    SELECT 
        RestaurantID,
        @SarahID,
        DATEADD(day, -4, GETDATE()),
        'The seafood was incredibly fresh. Beautiful ocean view!',
        4, 5, 5, 4,
        GETDATE()
    FROM dbo.TP_Restaurants WHERE Name = 'Miami Seafood Grill'
    UNION ALL
    SELECT 
        RestaurantID,
        @MikeID,
    ...
    SELECT 
        ...
        'The sushi was fresh and the service was outstanding!',
        5, 5, 4, 4,
        GETDATE()
    FROM dbo.TP_Restaurants WHERE Name = 'LA Sushi Bar'
    UNION ALL
    SELECT 
        ...
        'Excellent pasta and wonderful atmosphere.',
        4, 5, 5, 4,
        GETDATE()
    FROM dbo.TP_Restaurants WHERE Name = 'Chicago Pasta House';
END
